...
const getFavoritesErrorMessage = async (res: Response): Promise<string> => {
  try {
    const json = await res.json();
    return json.message || 'Något gick fel med favoriterna';
  } catch {
    return 'Något gick fel med favoriterna';
  }
};
